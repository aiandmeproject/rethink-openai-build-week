...
Rethink Core remains the shared PEC, STM, Claim Ledger, Evidence Lineage, Temporal Integrity, Reasoning Integrity, uncertainty, reducer, and Lab Notebook foundation. The universal Claim Ledger stores multiple explicit claims plus canonical `SUPPORTS`, `CONTRADICTS`, and `LIMITS` evidence relationships while keeping claims, assumptions, evidence, and the existing overall proposition behavior distinct. The universal Provenance Ledger separately records typed source/origin artifacts and directed lineage, rejects material dependency cycles, warns on citation cycles, and derives known Independent Evidence Chains only from explicit foundational roots. The universal Temporal Ledger records explicit `CURRENT`, `HISTORICAL`, `OUTDATED`, `CORRECTED`, `SUPERSEDED`, or `UNKNOWN` assessments plus acyclic `CORRECTS`/`SUPERSEDES` relationships and deterministic as-of warnings. The universal Reasoning Integrity Ledger attaches explicit claim-specific capability, scope, detection, and absence-inference assessments to stable Claim Ledger relationship IDs, then derives advisory warnings by composing the other Core ledgers. None of these analyses automatically changes claim status, relationship meaning, evidence authenticity, routing, Human Gates, proposition status, or human disposition. The versioned domain-profile registry also recognizes GENERAL, APPS, and NEWS as planned but unavailable; they are not selectable or operational in this branch. New and legacy projects resolve to BUSINESS unless an explicit supported profile is assigned.
...
The Provenance Ledger is a separate Core/data capability exposed through the same boundaries. Artifacts and relationships have stable identities and one canonical relationship source of truth. `DERIVED_FROM`, `SUMMARIZES`, `SYNDICATES`, and `REANALYZES` collapse derivative evidence to explicit `FOUNDATIONAL` roots; `CITES` and `REPLICATES` do not by themselves establish independence. Unknown, missing, partial, and citation-only lineage remains unresolved. Chain counts are derived rather than stored, synthetic roots remain traceable but ineligible for real-world validation, and provenance never changes a Claim Ledger status or proposition result.

The Temporal Ledger is a third, separate Core/data capability. One canonical assessment collection attaches explicit status and time fields to typed Evidence Items or provenance artifacts; one canonical relationship collection records correction/supersession direction from newer subject to affected object. Stable IDs, immutable endpoints, reason-gated updates, soft removal, interval validation, object-status consistency, and mixed-cycle rejection preserve auditability. Changing relationship meaning requires an explicit matching affected-assessment update in the same audited transaction, never an automatic status rewrite. Read-only target, source-chain, and claim analyses require an explicit `asOf`, distinguish current-state from historical-as-of purpose, and warn about stale, corrected, superseded, outdated, historical, or unresolved support. They never infer current status, rewrite a claim, alter Independent Evidence Chain counts, enable synthetic evidence, or change routing and Human Gates.

The Reasoning Integrity Ledger is a fourth Core/data capability. A Capability Assessment targets one stable Claim Ledger relationship ID and records explicit `UNKNOWN`, `FIT`, `PARTIAL`, or `NOT_FIT` overall fit; extensible scope dimensions; detection capability; absence-inference status; rationale; notes; and soft lifecycle history. The target cannot move, duplicate active assessments fail closed, and no claim/evidence endpoint copy is stored in the assessment. Claim and project analyzers require an explicit `asOf` and distinguish assessed from unassessed fit, scope mismatch, detection weakness, unsupported absence inference, Evidence Item count from foundational-root count, unresolved provenance and temporal applicability, synthetic/ineligible evidence, disconfirmation coverage, and advisory claim-status consistency warnings. Multiple documents never become multiple independent chains by counting alone; no recorded disconfirmation is not proof of no contradiction; and technical failure is not negative evidence. There is no numerical truth score, source weighting, automatic status change, profile-specific capability rule, autonomous disconfirmation search, or large Reasoning Integrity editor in this branch.

The active BUSINESS profile now registers a versioned Business Integrity policy and derives a read-only review from those Core analyses. It prioritizes target-market, customer-segment, geography, time, operating-context, endpoint/proxy, detection, provenance, independent-chain, temporal, disconfirmation, synthetic-evidence, public/private-data, and Human / Real-World boundaries. It reminds prompts and reports that interest is not purchase evidence, revenue is not profitability, public market data is not private operating data, and document count is not independent evidence count. The overlay adds no canonical ledger, score, automatic GO / NO-GO, route change, Claim change, proposition change, or Human Gate side effect. GENERAL, APPS, and NEWS remain unavailable and do not receive Business policy or vocabulary.

## Persistence and failure behavior

- Project state, Claim Ledger records, Provenance Ledger artifacts/relationships, Temporal Ledger assessments/relationships, Reasoning Integrity Capability Assessments, assumptions, evidence links, locked-version snapshots, reopening decisions, trace events, tangents, and notebook entries persist in browser `localStorage` on the current device.
...
The 201-test automated suite covers initialization; Claim, Provenance, Temporal, and Reasoning Integrity Ledger contracts and stable IDs; BUSINESS policy registration and profile isolation; deterministic derived Business review; market/customer/geography/time/endpoint scope; proxy and public/private evidence boundaries; Core warning composition; Evidence Item versus Independent Evidence Chain counting; Human / Real-World requirements; typed and immutable targets; explicit claim, temporal, fit, scope, detection, and absence-inference state; canonical many-to-many claim relationships; correction/supersession consistency and cycle rejection; deterministic explicit-as-of claim/project analysis; disconfirmation and claim-status consistency warnings; source-chain and temporal warnings; material provenance cycles and foundational-origin-role contradiction rejection; citation-cycle warnings; synthetic quarantine; legacy no-fabrication migration; v1/v2/resumable persistence; domain-profile contracts and active-profile prompt isolation; PEC phases; router and cycle schemas; project-context isolation; evidence participation and taxonomy; planned-test exclusion; source quality; prior-question reopening; validation-process versus proposition status; confidence trace; notebook continuity; linked Core/API CRUD; locks; background research and reasoning recovery; citation reconciliation; human gates; reports; loop detection; module/profile registration; repository migration; contractor acceptance and workforce isolation; credential exclusion; security headers; health/module endpoints; static serving; and HTTP end-to-end paths.
...
rethink-temporal.js       Versioned Temporal Ledger and deterministic as-of analysis
rethink-reasoning-integrity.js Versioned claim-specific capability ledger and advisory integrity analysis
rethink-business-integrity.js Versioned BUSINESS policy and derived read-only Core interpretation
...
This is a polished Build Week vertical slice and the first usable Rethink Core, not the complete future platform. Guided Mode is the priority. There is no database, account system, multi-project dashboard, collaborative workspace, autonomous multi-cycle runner, or production telemetry. Core now represents explicit provenance chains, derives known independent roots, records explicit temporal status and as-of warnings, and stores explicit claim-specific evidence capability/scope assessments with deterministic advisory analysis. BUSINESS applies a versioned derived integrity policy over that Core without adding mutable profile truth or a scoring engine. GENERAL, APPS, and NEWS remain unavailable. The platform does not implement source weighting, credibility scoring, numerical truth scoring, automatic claim-confidence/status proposals, autonomous disconfirmation research, or the complete Evidence-Origin Audit. OIP, CDIL, and OAMPES are routeable in Live Mode and represented by bounded Demo Mode behavior, but they are not expanded into separate research products.
