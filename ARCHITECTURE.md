...
Rethink Engine is the complete platform. Rethink Core is the shared domain-independent reasoning, Claim Ledger, Evidence Lineage, Temporal Integrity, Reasoning Integrity, evidence, uncertainty, integrity, and decision foundation. A Domain Profile is a versioned overlay that can add domain terminology, modules, and safeguards without copying PEC, STM, the Core ledgers, the Evidence Registry, reducers, the Lab Notebook, or the reasoning engine.
...
Project state includes the original input, current problem definition, current PEC phase, assumptions with status and confidence, a versioned Claim Ledger, Provenance Ledger, Temporal Ledger, and Reasoning Integrity Ledger, classified evidence, canonical evidence-to-claim/provenance/temporal relationships, locks, human gates, human decisions, stage overrides, tangents, notebook entries, and cycle metadata.
...
Legacy evidence remains valid under the existing proposition/evidence model but receives no fabricated Claim Ledger link. Claim Ledger state is included in compact project prompts, lock snapshots, reports, notebook exports, project backups, imports, device-local sessions, and resumable v2 backups. Prompt instructions keep claims, assumptions, evidence, and cycle-level proposition status distinct.

### Core Evidence Lineage and Provenance Ledger

`rethink-provenance.js` defines one universal, versioned Provenance Ledger. `artifacts` represent explicitly recorded sources or origins with stable IDs, extensible uppercase kinds, `FOUNDATIONAL`, `DERIVATIVE`, or `UNKNOWN` origin roles, optional attribution/locator fields, notes, and ordinary identity timestamps. `relationships` are stored once, outside Evidence Items and artifacts, and use typed `EVIDENCE_ITEM` or `PROVENANCE_ARTIFACT` endpoints. The direction is always subject/child to object/parent or referenced origin.

The relationship registry contains `DERIVED_FROM`, `CITES`, `SUMMARIZES`, `REPLICATES`, `REANALYZES`, and `SYNDICATES`. `DERIVED_FROM`, `SUMMARIZES`, `SYNDICATES`, and `REANALYZES` are material dependencies: their active graph must be acyclic and every known chain terminates at an artifact explicitly marked `FOUNDATIONAL`. A `FOUNDATIONAL` artifact cannot be the subject/child of an active material dependency, though it may be the object/parent and may participate in `CITES` or `REPLICATES`; removed historical material relationships do not impose a current ancestry constraint. Direct and multi-node material cycles and foundational-role contradictions fail closed during mutation, project normalization, and backup import. Citation cycles are valid historical/source structures, so they remain stored and surface deterministic warnings instead of being rewritten as material dependency cycles. `REPLICATES` does not collapse two sources; a replication counts separately only when its Evidence Item reaches its own explicitly foundational evidence-generation artifact.

Independent Evidence Chain analysis is a deterministic projection, never stored project state. Multiple Evidence Items that reach one foundational root form one known chain; one derivative with multiple foundational parents maps to all of those roots. Unlinked Evidence Items, citation-only paths, `UNKNOWN` artifacts without material parents, and orphan `DERIVATIVE` artifacts remain `UNRESOLVED` or `PARTIAL`. Titles, URLs, authors, publishers, wording, record counts, and citation counts never create inferred lineage or independence.

Claim-level analysis selects only active Claim Ledger evidence relationships and active, linkable Evidence Items, then derives roots through the separate Provenance Ledger. It never changes claim status and never turns `SUPPORTS`, `CONTRADICTS`, or `LIMITS` counts into independent-chain counts. Removed evidence is excluded from active analysis while its retained Evidence Item and provenance relationships remain inspectable. Synthetic provenance is traceable, but its roots are excluded from the eligible real-world chain count and cannot affect proposition or Human Gate eligibility.

Reason-gated reducer operations create/update artifacts, create/update relationship meaning or notes without endpoint reassignment, and soft-remove relationships. Every mutation creates the existing `stateEvents` and `STATE_EDIT` Notebook audit records. Duplicate identical relationships are idempotent, missing typed endpoints fail closed, and material cycles cannot enter canonical state. Legacy projects receive an empty ledger only; existing source metadata is not converted into artifacts by inference. The ledger and derived read-only summaries pass through bounded relevant prompts, lock snapshots, reports, notebook export, v1/v2 backup/import, device-local persistence, and resumable state without changing backup format versions.

### Core Temporal Integrity

`rethink-temporal.js` defines one universal, versioned Temporal Ledger, separate from the Claim and Provenance Ledgers. Its authoritative `assessments` collection attaches one active explicit status to a typed `EVIDENCE_ITEM` or `PROVENANCE_ARTIFACT`: `UNKNOWN`, `CURRENT`, `HISTORICAL`, `OUTDATED`, `CORRECTED`, or `SUPERSEDED`. Assessments record `statusAsOf`, optional observed/published/effective dates, rationale, notes, lifecycle state, and stable identity timestamps. No temporal copy is embedded in evidence, claims, citations, or provenance artifacts.

The authoritative `relationships` collection records `CORRECTS` and `SUPERSEDES` from the newer/correcting subject to the older/affected object. Active relationships require existing typed endpoints, a matching active object assessment (`CORRECTED` or `SUPERSEDED`), and an `effectiveAt` no later than the affected assessment's `statusAsOf`. Endpoints are immutable, duplicates are idempotent, removal is soft, and the active mixed correction/supersession graph must be acyclic. A meaning change must carry an explicit matching affected-assessment update in the same audited transaction so no contradictory intermediate state is accepted; the reducer never changes that assessment implicitly. Removed assessments and relationships remain historical records. Contradictory normalization and v1/v2 import fail closed; the reducer never fabricates a replacement, rewrites a status, or deletes history to repair a conflict.

Temporal analysis is a deterministic projection and always requires an explicit `asOf` plus `CURRENT_STATE` or `HISTORICAL_AS_OF` purpose. It reports assessed/unassessed targets, exact status groups, interval mismatches, relationship conflicts, unresolved correcting/superseding sources, replacement chains, and inspect-only cycle warnings. Stored status is never recalculated from the analysis date. Missing state remains unresolved rather than becoming current; an old source is not automatically false, and a newer source is not automatically better.

Source-chain analysis composes the Temporal and Provenance Ledgers without merging them: provenance answers where an item came from, while temporal state answers when the evidence, derivative artifacts, and foundational roots were explicitly assessed. Claim-level analysis selects only active Claim Ledger relationships and active linkable Evidence Items, preserves `SUPPORTS`/`CONTRADICTS`/`LIMITS`, and emits warnings such as current support relying on corrected, superseded, outdated, historical, or unresolved sources. These warnings do not change claim status, evidence eligibility, proposition status, Independent Evidence Chain counts, routing, or Human Gates. Synthetic evidence may be assessed but remains synthetic and ineligible.

Reason-gated reducer operations create/update and soft-remove assessments and relationships through the existing state-event and Notebook boundary. Read-only target, source-chain, and claim analyzers accept caller-supplied `asOf`. Prompt context contains only bounded active-project temporal records and an explicit reproducible timestamp (the canonical project `updatedAt` unless a caller supplies one). Reports use their explicit generation timestamp. New and legacy projects receive an empty ledger; source dates, retrieval dates, text, and machine time never fabricate assessments. Temporal state passes through progress signatures, cycles, locks, Notebook export, structured/human reports, device-local sessions, v1/v2 backup/import, and resumable state without a backup-version bump.

### Core Reasoning Integrity

`rethink-reasoning-integrity.js` defines one universal, versioned Reasoning Integrity Ledger. Its canonical `capabilityAssessments` collection targets stable Claim Ledger relationship IDs, so capability is specific to an Evidence Item's `SUPPORTS`, `CONTRADICTS`, or `LIMITS` relationship with one claim. Claims and Evidence Items do not receive independently mutable capability copies. Assessment IDs are stable, targets are immutable, only one active assessment may target a relationship, and removal is soft. New assessments require an active Claim Ledger relationship; assessments attached to relationships later retired remain inspectable historical state.

Each assessment records explicit `UNKNOWN`, `FIT`, `PARTIAL`, or `NOT_FIT` overall fit; extensible uppercase scope dimensions; explicit detection capability; explicit absence-inference status; rationale, notes, lifecycle state, and audit timestamps. Scope dimensions compare claim and evidence scope using `MATCHED`, `PARTIAL`, `MISMATCHED`, `UNKNOWN`, or `NOT_APPLICABLE`. Canonical validation rejects duplicate dimensions, malformed tokens, `FIT` with an explicit mismatch, inconsistent material detection, unsupported absence-inference combinations, unknown relationship targets, duplicate active assessments, and target reassignment. Overall fit is never calculated from dimension counts.

Read-only claim analysis composes the Claim, Provenance, Temporal, and Reasoning Integrity Ledgers at an explicit `asOf`. It reports assessed/unassessed relationships, fit groups, scope mismatch, detection and absence-inference weaknesses, Evidence Item count versus known foundational-root count by relationship type, unresolved origin and temporal state, synthetic/ineligible evidence, disconfirmation coverage, and advisory claim-status consistency warnings. Project analysis aggregates those deterministic results without truth scores, source-credibility weights, claim ranking, automatic research, or project-disposition changes.

The integrity boundary preserves four distinctions: multiple documents may collapse to one Independent Evidence Chain; absence of detection requires explicit adequate detection capability before it can support absence; no recorded contradiction does not prove no contradiction exists; and technical failure is not a substantive negative finding. `NOT_FIT`, scope warnings, disconfirmation gaps, and status-consistency warnings never automatically rewrite a Claim, Claim Ledger relationship, proposition status, routing decision, Human Gate, or human disposition. Synthetic evidence may be assessed but remains synthetic and ineligible for real-world validation.

Reason-gated reducer operations create/update and soft-remove Capability Assessments through the existing state-event and Notebook boundary. Model cycle output cannot create capability state. Prompts receive bounded active-project assessments and derived warnings with an explicit analysis timestamp; reports distinguish stored state from derived analysis. New and legacy projects receive an empty ledger without fabricated fit, scope, detection, absence, disconfirmation, timestamps, events, or Notebook records. Capability state passes through cycles, locks, Notebook export, structured/human reports, device-local sessions, v1/v2 backup/import, and resumable state without a backup-version bump.
...
The notebook is appended, never rewritten by a later cycle. Manual claim, claim-evidence relationship, provenance artifact/relationship, temporal assessment/relationship, Capability Assessment, assumption, evidence, and locked-version actions also append a `STATE_EDIT` notebook record and a structured `stateEvents` before/after trace without incrementing the reasoning cycle. A browser repository adapter holds the canonical device-local active session in `localStorage`. Versioned project backup/import and focused Notebook export provide portable records.
...
Lock It In snapshots the current problem definition, PEC phase, assumptions, Claim Ledger, Provenance Ledger, Temporal Ledger, Reasoning Integrity Ledger, and evidence. Locked versions can be inspected and compared against the current canonical state. Reopening never rolls the project backward: it marks the checkpoint reopened and requires a recorded trigger and rationale. Valid triggers include new evidence, a failed critical assumption, an integration conflict, or a demonstrably better version.
...
`public/project-repository.js` is the first persistence interface. The v0.1 adapter exposes load, save, and clear for one active device-local session and migrates the legacy storage key. Missing legacy Claim, Provenance, Temporal, and Reasoning Integrity Ledger state hydrates to versioned empty forms without claims, links, artifacts, assessments, relationships, capability conclusions, or timestamps. Project Backup v2 contains the complete normalized project plus safe resumable UI/runtime job envelopes, but never credentials. Import accepts v1 and v2, validates the project and all four ledgers, records the import event, and restores it under the destination origin. Human-readable Report HTML, Report JSON, and Notebook JSON are intentionally separate from the restorable backup.
